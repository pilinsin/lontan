...
func loadImage(r io.Reader) (fyne.CanvasObject, error){
	...
	pbImage := &pb.Image{}
	if err := proto.Unmarshal(buf.Bytes(), pbImage); err != nil{return nil, err}

	name := pbImage.GetName()
	data := pbImage.GetData()
	if data == nil{return nil, errors.New("invalid image")}
...
func LoadImage(cid string, is ipfs.Ipfs) fyne.CanvasObject{
	...
	if err != nil{return errorLabel("load image error (ipfs)")}

	img, err := loadImage(r)
	if err != nil{return errorLabel("load image error")}
	return img
}

func LoadText(cid string, is ipfs.Ipfs) fyne.CanvasObject{
	r, err := is.GetReader(cid)
	if err != nil{return errorLabel("load text error (ipfs)")}
	buf := &bytes.Buffer{}
	_, err = buf.ReadFrom(r)
	if err != nil{return errorLabel("load text error")}
...
func LoadAudio(cid string, is ipfs.Ipfs) fyne.CanvasObject{
	...
	if err != nil{return errorLabel("load audio error")}
	return ap.Render()
}
func LoadVideo(cid string, is ipfs.Ipfs) fyne.CanvasObject{
	vp, err := NewVideoPlayer(cid, is)
	if err != nil{return errorLabel("load video error")}
	return vp.Render()
}

func LoadPdf(cid string, is ipfs.Ipfs) fyne.CanvasObject{
	m, err := is.Get(cid)
	if err != nil{return errorLabel("load pdf error (ipfs)")}
	...
	for idx, mImg := range mImgs{
		...
		if err != nil{return errorLabel("load pdf error")}
