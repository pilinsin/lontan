...
func NewVideoPlayer(cid string, is ipfs.Ipfs) (*videoPlayer, error){
	...
	pbVideo := &pb.Video{}
	if err := proto.Unmarshal(m, pbVideo); err != nil{return nil, err}
	...
	if err := vp.init(); err != nil{return nil, err}
...
func (vp *videoPlayer) init() error{
	if err := vp.speakerInit(); err != nil{return err}
	...
	onChanged := func(val float64){
		...
		if idx != vp.chunkIndex{
			vp.Clear()
			if err := vp.speakerInit(); err != nil{return}
			vp.ctx, vp.cancel = context.WithCancel(context.Background())
			if err := vp.LoadChunk(idx); err != nil{return}
			vp.Play()
		}
	}
...
func (vp *videoPlayer) Close(){
	...
	speaker.Clear()
	speaker.Close()
	if vp.ticker != nil{vp.ticker.Stop()}
	vp.is = nil
}
...
func (vp *videoPlayer) LoadChunk(idx int) error{
	if idx == vp.chunkIndex{return nil}
...
func (vp *videoPlayer) newBeepStreamer() beep.Streamer{
	return beep.StreamerFunc(func(samples [][2]float64) (int, bool){
		numRead := 0
		if vp.chunk == nil{return numRead, false}
...
func (vp *videoPlayer) updateSprite() error{
	if vp.chunk == nil{return fmt.Errorf("chunk is nil")}
	for{
		if vp.paused{
			time.Sleep(time.Millisecond*8)
			continue
...
func (vp *videoPlayer) Play(){
	...
	go func(){
		for{
			...
			vp.updateTimeBar()
			if err := vp.updateSprite(); err != nil{break}//vp.cancel() from outside
			vp.cancel()
			vp.ctx, vp.cancel = context.WithCancel(context.Background())
			if err := vp.LoadChunk(vp.chunkIndex+1); err != nil{break}
		}
		vp.paused = true
		vp.playing = false
	}()
