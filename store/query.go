...
func (o TimeOrder) Compare(a, b query.Entry) int {
	da := newEmptyDocument()
	if err := da.Unmarshal(a.Value); err != nil {
		return 1
	}
	db := newEmptyDocument()
	if err := db.Unmarshal(b.Value); err != nil {
		return -1
	}
...
func (f CidsFilter) Filter(e query.Entry) bool {
	d := newEmptyDocument()
	if err := d.Unmarshal(e.Value); err != nil {
		return false
	}
